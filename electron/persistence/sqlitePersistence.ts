...
const localBackupService = createLocalBackupService({
  getDatabase: () => database,
  getStatus: getSqliteStatus,
  acquireLock,
  releaseLock,
  ...
  migrateLocalStorageSnapshot: (payload) => migrateLocalStorageSnapshot(payload),
  withDatabaseOperationLock,
  backupExistingDatabase,
  closeDatabaseAndReleaseLock,
  activateDatabase: (directoryPath, isDefaultPath, seedFromDatabasePath) =>
    activateDatabase(directoryPath, isDefaultPath, seedFromDatabasePath),
});

function enqueueLocalBackup(reason: string): void {
  localBackupService.enqueueLocalBackup(reason);
}

async function flushPendingLocalBackup(): Promise<void> {
  await localBackupService.flushPendingLocalBackup();
}

export async function createShutdownLocalBackup(): Promise<void> {
  await localBackupService.createShutdownLocalBackup();
}

export async function createManualLocalBackup(): Promise<void> {
  await localBackupService.createManualLocalBackup();
}
...
export async function listLocalBackups(): Promise<LocalBackupEntry[]> {
  return localBackupService.listLocalBackups();
}

export async function restoreLocalBackup(fileName: string): Promise<RestoreLocalBackupResult> {
  return localBackupService.restoreLocalBackup(fileName);
}
