...
export function DataTable<Row, ColumnId extends string>({
  ...
  return (
    <div className="space-y-2">
      <div
        className={`relative ${heightClassName ?? ''} ${maxHeightClassName} overflow-auto rounded-xl border border-metro-border/80 bg-metro-surface/70 shadow-sm shadow-slate-950/15`}
        ref={scrollContainerRef}
      >
        {onResetColumnWidths && (
          <button
            aria-label="Restablecer columnas"
            className="absolute right-1 top-1 z-20 inline-flex h-7 w-7 items-center justify-center rounded-lg border border-transparent bg-metro-panel/95 text-metro-muted transition-colors hover:border-metro-border hover:text-metro-text"
      ...
      {hasHiddenRows && (
        <div className="flex items-center justify-between rounded-lg border border-metro-border/70 bg-metro-panel/45 px-3 py-1.5 text-xs text-metro-muted">
          <span>
