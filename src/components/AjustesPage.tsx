...
export function AjustesPage() {
  ...
  const handleSelectSchoolHelpFolder = async () => {
    setStatus('');
    if (!window.traccion?.selectSchoolHelpFolder) {
      setStatus('El selector de carpetas solo está disponible en la aplicación de escritorio.');
      return;
    }
    const selectedPath = await window.traccion.selectSchoolHelpFolder();
    if (!selectedPath) return;
    setRutaAyudaEscolar(selectedPath);
    setStatus('Carpeta de Ayuda escolar guardada.');
  };
  ...
  return (
    <section className="rounded-3xl border border-metro-border bg-metro-surface p-5 shadow-card">
      ...
      <div className="mb-4 rounded-2xl border border-metro-border bg-metro-panel p-4">
        <div className="mb-4">
          <h3 className="text-base font-bold text-metro-text">Ayuda escolar</h3>
          <p className="mt-1 text-sm text-metro-muted">
            Carpeta donde se archivarán los adjuntos recibidos desde Outlook. Puede ser una ruta local, UNC o una unidad de red mapeada.
          </p>
        </div>
        ...
        <label className="block text-xs font-semibold text-metro-muted">
          ...
          <input
            className="mt-1 w-full rounded-lg border border-metro-border bg-metro-surface px-3 py-2 text-sm font-medium text-metro-text outline-none focus:border-metro-red"
            onChange={(event) => setRutaAyudaEscolar(event.target.value)}
            placeholder="G:\\Capital Humano\\...\\Ayuda Escolar"
            type="text"
        ...
        <div className="mt-3 flex flex-wrap items-center gap-3">
          <button
            className="inline-flex items-center gap-2 rounded-lg bg-metro-red px-3 py-2 text-sm font-semibold text-white hover:bg-metro-dark"
            onClick={() => void handleSelectSchoolHelpFolder()}
            type="button"
          >
            <FolderOpen size={16} />
            Seleccionar carpeta
