...
type SortKey = 'empleado' | 'nombreApellidos' | 'email' | 'puestoNomina' | 'residencia' | 'nivelRetributivo';
...
const sortableColumns: Array<{ key: SortKey; label: string; className: string }> = [
  ...
  { key: 'nombreApellidos', label: 'Nombre y apellidos', className: 'w-[210px]' },
  { key: 'email', label: 'Correo', className: 'w-[210px]' },
            ...
            <tbody className="divide-y divide-metro-border bg-metro-surface">
              {sortedEmployees.map((employee) => (
                ...
                >
                  ...
                  <td className="truncate px-3 py-1.5 text-metro-muted" title={employee.email || '—'}>
                    {employee.email || '—'}
                  </td>
                  <td
