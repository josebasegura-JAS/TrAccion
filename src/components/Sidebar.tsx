...
const readStoredActiveGroup = (activeView: AppView) => {
  if (typeof window === 'undefined') return getGroupForView(activeView) ?? 'personas';
...
export function Sidebar({
  ...
  const handleLogoSelect = () => {
    ...
    if (onDashboardReset) { onDashboardReset(); return; }
    onViewChange('dashboard');
  };
  const handleHomeSelect = () => { onViewChange('dashboard'); if (!isPinned) setIsPanelOpen(false); };
  const handleGroupSelect = (groupId: NavigationGroupId) => { setActiveGroupId(groupId); setIsPanelOpen(true); };
  const handleSettingsSelect = () => { onViewChange('ajustes'); if (!isPinned) setIsPanelOpen(false); };
  const handleDatabaseIndicatorSelect = () => {
    if (typeof window !== 'undefined') window.location.hash = 'base-de-datos';
    ...
    <aside className={`relative z-30 h-screen shrink-0 transition-[width] duration-300 ease-out ${shouldShowPanel && isPinned ? 'w-[22rem]' : 'w-[4.5rem]'}`}>
          ...
          <div className="mt-auto flex flex-col items-center gap-2 border-t border-white/10 pt-3">
            <button aria-label={`Estado de base de datos: ${databaseIndicator.statusText}`} className="group/rail relative flex w-12 flex-col items-center justify-center gap-1 rounded-xl border border-white/10 bg-white/[0.035] px-1 py-2 text-[11px] font-semibold text-slate-300 transition hover:bg-white/[0.07] hover:text-white" data-tip={shouldShowPanel ? databaseIndicatorTooltip : undefined} onClick={handleDatabaseIndicatorSelect} type="button">
              ...
              <span className={`pointer-events-none absolute left-14 z-50 w-72 rounded-lg border border-white/10 bg-slate-950/95 px-3 py-2 text-left text-xs font-medium normal-case tracking-normal text-slate-100 opacity-0 shadow-xl shadow-slate-950/40 transition ${shouldShowPanel ? 'hidden' : 'group-hover/rail:translate-x-1 group-hover/rail:opacity-100'}`}>
                ...
                <span className={`mt-1 block ${persistenceFeedback?.kind === 'error' ? 'text-red-300' : pendingWriteCount > 0 || persistenceFeedback?.kind === 'saving' ? 'text-amber-300' : 'text-emerald-300'}`}>{persistenceStatusText}</span>
      ...
      {shouldShowPanel && (
        <div className="fixed inset-y-0 left-[4.5rem] z-30 flex w-[17.5rem] flex-col border-r border-slate-700/80 bg-gradient-to-b from-[#0f172a] via-[#101b30] to-[#0b1425] text-white shadow-2xl shadow-slate-950/45 backdrop-blur-xl transition-transform duration-300 ease-out">
          ...
          <div className="border-b border-white/10 px-4 py-4"><p className="text-sm leading-5 text-slate-300">{activeGroup.description}</p></div>
          <nav aria-label={`Opciones de ${activeGroup.label}`} className="flex-1 space-y-1 px-3 py-4">
            {activeGroup.items.map((item) => {
              ...
              return (
                <button aria-current={isActiveItem ? 'page' : undefined} className={`relative flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-medium transition ${isActiveItem ? 'bg-white/10 text-metro-text shadow-sm shadow-slate-950/20' : 'text-slate-300 hover:bg-white/[0.05] hover:text-white'} ${item.disabled ? 'cursor-not-allowed opacity-45 hover:bg-transparent hover:text-slate-300' : ''}`} disabled={item.disabled} key={item.label} data-tip={item.disabled ? 'Módulo no disponible en este paquete' : undefined} onClick={() => item.view && handleViewSelect(item.view)} type="button">
                  {isActiveItem && <span className="absolute left-0 h-6 w-0.5 rounded-r-full bg-metro-red" />}
