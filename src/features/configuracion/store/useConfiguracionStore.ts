...
interface ConfiguracionStore extends ConfiguracionState {
  ...
  setRutaAyudaEscolar: (ruta: string) => void;
...
function isConfiguracionState(value: unknown): value is ConfiguracionState {
  ...
  return (
    typeof candidate.rutaPlantillaTeletrabajo === 'string' &&
    (candidate.rutaAyudaEscolar === undefined || typeof candidate.rutaAyudaEscolar === 'string')
  );
}
...
function readConfiguracion(): ConfiguracionState {
  ...
  if (!stored) {
    return { rutaPlantillaTeletrabajo: '', rutaAyudaEscolar: '', taskPhases: DEFAULT_TASK_PHASES };
  }
  ...
  return {
    ...
    rutaAyudaEscolar: typeof parsed.rutaAyudaEscolar === 'string' ? parsed.rutaAyudaEscolar.trim() : '',
...
export const useConfiguracionStore = create<ConfiguracionStore>((set) => ({
  ...
  setRutaAyudaEscolar: (ruta) =>
    set((state) => {
      const configuracion = { ...state, rutaAyudaEscolar: ruta.trim() };
      persistConfiguracion(configuracion);
      return configuracion;
    }),
  addTaskPhase: (nombre) =>
