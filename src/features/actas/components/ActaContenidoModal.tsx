...
export function ActaContenidoModal({ acta, onClose }: { acta: Acta; onClose: () => void }) {
  ...
  const activeCenso = useMemo(() => selectActiveCensoMiembros(censo, acta.tipo), [censo, acta.tipo]);
  ...
  const sindicatosDelCenso = useMemo(
    () =>
      activeCenso
        ...
        .filter(
          (miembro, index, all) => all.findIndex((m) => m.organizacion === miembro.organizacion) === index,
        ),
    [activeCenso],
  );
  ...
  const hasControlPreviaWarnings =
    unresolvedPuntos.length > 0 || incompleteAcuerdos.length > 0 || votacionesConPendientes.length > 0;
      ...
      <div className="flex-1 space-y-4 overflow-y-auto px-4 py-4">
            ...
            <div className="flex items-start gap-2">
              ...
              <ul className="list-disc space-y-0.5 pl-4 font-normal normal-case">
                {unresolvedPuntos.length > 0 && (
                  <li>
                    {unresolvedPuntos.length} punto{unresolvedPuntos.length > 1 ? 's' : ''} del día sin resultado (
                    {unresolvedPuntos.map((p) => p.titulo).join(', ')}).
                  </li>
                )}
                {incompleteAcuerdos.length > 0 && (
                  <li>
                    {incompleteAcuerdos.length} acuerdo{incompleteAcuerdos.length > 1 ? 's' : ''} sin responsable o
                    sin fecha límite.
                  </li>
                )}
                {votacionesConPendientes.length > 0 && (
                  <li>
                    {votacionesConPendientes.length} votación{votacionesConPendientes.length > 1 ? 'es' : ''} con
                    alguna posición pendiente ({votacionesConPendientes.map((v) => v.tema).join(', ')}).
                  </li>
                )}
              </ul>
            </div>
        ...
        <section className={sectionClassName()}>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <label className="text-xs font-semibold text-metro-muted">
              Hora inicio
              <Input
                onChange={(event) => void commit((c) => ({ ...c, horaInicio: event.target.value }))}
                type="time"
            ...
            <label className="text-xs font-semibold text-metro-muted">
              Hora fin
              <Input
                onChange={(event) => void commit((c) => ({ ...c, horaFin: event.target.value }))}
                type="time"
          ...
          <div className="space-y-2">
            ...
            {contenido.recesos.map((receso) => (
              <div className="flex items-center gap-2" key={receso.id}>
                <Input
                  onChange={(event) => void commit(withUpdatedReceso(receso.id, { horaInicio: event.target.value }))}
                  type="time"
                  value={receso.horaInicio}
                />
                <span className="text-metro-muted">–</span>
                <Input
                  onChange={(event) => void commit(withUpdatedReceso(receso.id, { horaFin: event.target.value }))}
                  type="time"
        ...
        <section className={sectionClassName()}>
          ...
          {activeCenso.length === 0 && (
            <Notice tone="info">
              No hay censo dado de alta para {acta.tipo}. Da de alta a los miembros desde &quot;Gestionar censo&quot;
              para poder pasar lista.
            </Notice>
          )}
                  ...
                  {asistenciaByGrupo[grupo].map((entry) => (
                    ...
                    >
                      <span className="truncate text-sm text-metro-text" title={entry.nombre}>
                        {entry.nombre}
                        {entry.organizacion && (
                          <span className="ml-1 text-xs text-metro-muted">({entry.organizacion})</span>
                        )}
                      </span>
                      <Select
                        onChange={(event) =>
                      ...
                      <Input
                        onChange={(event) =>
                          void commit(withUpdatedAsistenciaEntry(entry.id, { horaEntrada: event.target.value }))
                        }
                        placeholder="Entrada"
                        type="time"
                      ...
                      <Input
                        onChange={(event) =>
                          void commit(withUpdatedAsistenciaEntry(entry.id, { horaSalida: event.target.value }))
                        }
                        placeholder="Salida"
                        type="time"
        ...
        <section className={sectionClassName()}>
          ...
          <div className="space-y-3">
            {contenido.puntos.map((punto, index) => (
              <div className="space-y-2 rounded-lg border border-metro-border bg-metro-surface p-3" key={punto.id}>
                <div className="flex items-center gap-2">
                  <span className="shrink-0 text-xs font-bold text-metro-muted">{index + 1}.</span>
                  <Input
                    className="flex-1"
                    onChange={(event) => void commit(withUpdatedPunto(punto.id, { titulo: event.target.value }))}
                    value={punto.titulo}
                  />
                  <Select
                    className="w-48 shrink-0"
                    onChange={(event) =>
                      void commit(withUpdatedPunto(punto.id, { resultado: event.target.value as ActaPuntoResultado }))
                    }
                ...
                <div className="flex gap-2">
                  ...
                  <ActionButton
                    onClick={() =>
                      void commit(withAddedVotacion({ puntoId: punto.id, tema: punto.titulo, posiciones: [] }))
                    }
                    size="sm"
            ...
            {contenido.acuerdos.map((acuerdo) => {
              ...
              return (
                <div
                  ...
                  key={acuerdo.id}
                >
                  <p className="text-xs font-semibold text-metro-muted">{punto?.titulo ?? 'Punto eliminado'}</p>
                  <Textarea
                    onChange={(event) =>
                      void commit(withUpdatedAcuerdo(acuerdo.id, { descripcion: event.target.value }))
                    }
                  ...
                  <div className="grid gap-2 sm:grid-cols-3">
                    <Input
                      onChange={(event) =>
                        void commit(withUpdatedAcuerdo(acuerdo.id, { responsable: event.target.value }))
                      }
                    ...
                    <Input
                      onChange={(event) =>
                        void commit(withUpdatedAcuerdo(acuerdo.id, { fechaLimite: event.target.value }))
                      }
                      type="date"
                    ...
                    <div className="flex items-center gap-2">
                      <Select
                        onChange={(event) =>
                          void commit(
                            withUpdatedAcuerdo(acuerdo.id, { estado: event.target.value as ActaAcuerdoEstado }),
                          )
                        }
              ...
              return (
                ...
                >
                    ...
                    {(Object.keys(summary) as VotacionPosicion[])
                      ...
                      .map((posicion) => (
                        <StatusBadge
                          key={posicion}
                          size="xs"
                          tone={posicion === 'favor' ? 'success' : posicion === 'contra' ? 'error' : 'muted'}
                        >
                  ...
                  {sindicatosDelCenso.length === 0 ? (
                    <Notice tone="info">
                      No hay sindicatos dados de alta en el censo — da de alta el censo para poder registrar
                      posiciones de voto.
                    </Notice>
                  ) : (
                    <div className="space-y-1.5">
                      {sindicatosDelCenso.map((miembro) => {
                        ...
                        return (
                          <VotacionPosicionRow
                            ...
                            onChange={(patch) =>
                              void commit(withUpsertedVotacionPosicion(votacion.id, miembro.organizacion, patch))
                            }
...
function VotacionPosicionRow({
  ...
  onChange: (patch: { posicion: VotacionPosicion; fecha?: string | null; observacion?: string }) => void;
  ...
  return (
    <div className="grid grid-cols-[100px_150px_1fr] items-center gap-2">
      ...
      <Input
        onChange={(event) => onChange({ posicion: current.posicion, observacion: event.target.value })}
