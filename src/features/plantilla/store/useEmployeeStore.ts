...
export const useEmployeeStore = create<EmployeeState>((set, get) => ({
  ...
  updateEmail: async (empleado, email) => {
    const normalizedEmail = email.trim().toLowerCase();
    if (!normalizedEmail) return { ok: false, message: 'El correo electrónico está vacío.' };
    const state = get();
    const employee = state.employees.find((item) => item.empleado === empleado && !item.deletedAt);
    if (!employee) return { ok: false, message: 'No se ha encontrado la persona en Plantilla.' };
    const currentEmail = (employee.email ?? '').trim().toLowerCase();
    if (currentEmail === normalizedEmail) return { ok: true, message: 'El correo ya estaba asociado a la persona.' };
    if (currentEmail) return { ok: false, message: `La persona ya tiene otro correo (${employee.email}). No se ha sobrescrito.` };
    const duplicate = state.employees.find((item) => !item.deletedAt && item.empleado !== empleado && (item.email ?? '').trim().toLowerCase() === normalizedEmail);
    if (duplicate) return { ok: false, message: `Ese correo ya está asociado a ${duplicate.nombreApellidos}.` };
    ...
    return get().updateWithConcurrencyCheck(employee.empleado, draft, JSON.stringify(employee));
  },
