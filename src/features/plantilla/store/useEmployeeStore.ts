...
interface EmployeeState {
  ...
  updateEmail: (empleado: string, email: string) => void;
...
function upsertEmployees(current: Employee[], drafts: EmployeeDraft[]): Employee[] {
  ...
  drafts.forEach((draft) => {
    ...
    const nextDraft = { ...draft, email: draft.email.trim() || previous?.email || '' };
...
export const useEmployeeStore = create<EmployeeState>((set, get) => ({
  ...
  updateEmail: (empleado, email) => {
    const normalizedEmail = email.trim().toLowerCase();
    if (!normalizedEmail) return;
    set((state) => {
      const employees = state.employees.map((employee) =>
        employee.empleado === empleado
          ? hydrateEmployee({ ...employee, email: normalizedEmail }, employee.deletedAt)
          : employee,
      );
      persistEmployees(employees);
      return { employees };
