...
const HEADER_ALIASES: ReadonlyArray<readonly [EmployeeField, readonly string[]]> = [
  ...
  ['email', ['email', 'e-mail', 'correo', 'correo electronico', 'correo electrónico', 'mail', 'direccion email', 'dirección email']],
  ...
  ['calle', ['calle', 'direccion', 'dirección', 'domicilio', 'direccion postal', 'dirección postal', 'direccion particular', 'dirección particular']],
...
export async function analyzeEmployeeImportFile(file: File): Promise<EmployeeImportPreview> {
  ...
  const defaultMapping = headers.map((header) =>
    sourceProfile === 'zerkos'
      ? (ZERKOS_FIELD_BY_HEADER.get(normalizeHeader(header)) ?? FIELD_BY_HEADER.get(normalizeHeader(header)) ?? null)
