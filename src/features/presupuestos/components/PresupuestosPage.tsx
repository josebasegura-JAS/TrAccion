...
export function PresupuestosPage() {
  ...
  const scenarioColumns = useMemo<Array<DataTableColumn<BudgetScenario, ScenarioColumnId>>>(() => [
    ...
    { id: 'actions', header: 'Acciones', width: 210, isActionColumn: true, render: (row) => <div className="flex justify-end gap-1"><ActionButton onClick={(event) => { event.stopPropagation(); setActiveScenario(row.id); }} size="sm" title="Seleccionar escenario" type="button" variant="secondary" iconOnly={false}>Seleccionar</ActionButton><ActionButton onClick={(event) => { event.stopPropagation(); setScenarioDraft({ name: row.name, year: row.year, ticketAmount: row.ticketAmount, notes: row.notes }); setEditingScenarioId(row.id); }} size="sm" title="Editar escenario" type="button" variant="edit" /><ActionButton onClick={(event) => { event.stopPropagation(); duplicateScenario(row.id); }} size="sm" title="Duplicar escenario" type="button" variant="duplicate" /><ActionButton onClick={(event) => { event.stopPropagation(); removeScenario(row.id); }} size="sm" title="Eliminar escenario" type="button" variant="delete" /></div> },
