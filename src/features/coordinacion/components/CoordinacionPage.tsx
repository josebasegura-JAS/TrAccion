...
export function CoordinacionPage() {
  ...
  return <section className="space-y-4">
    ...
    <div className="grid gap-3 md:grid-cols-3">
      <button className={`rounded-2xl border p-4 text-left ${area === 'direccion' ? 'border-metro-red bg-metro-panel' : 'border-metro-border bg-metro-surface'}`} onClick={() => setArea('direccion')} type="button"><Building2 className="mb-3 text-metro-red" size={22}/><strong className="block text-metro-text">Dirección</strong><span className="text-xs text-metro-muted">Tareas marcadas, nuevas tareas y puntos libres</span></button>
      <button className={`rounded-2xl border p-4 text-left ${area === 'otras-areas' ? 'border-metro-red bg-metro-panel' : 'border-metro-border bg-metro-surface'}`} onClick={() => setArea('otras-areas')} type="button"><UsersRound className="mb-3 text-metro-red" size={22}/><strong className="block text-metro-text">Otras áreas</strong><span className="text-xs text-metro-muted">Tareas existentes, nuevas y puntos libres</span></button>
