...
function meetingContext(meeting: CoordinationMeeting): string {
  return meeting.area === 'direccion' ? 'Dirección' : meeting.areaName?.trim() || 'otra área';
}
    ...
    return <section className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          {!isDirection && <p className="mt-1 text-sm text-metro-muted">{selected.interlocutors ? `Interlocutores: ${selected.interlocutors}` : 'Sin interlocutores indicados'}{selected.purpose ? ` · ${selected.purpose}` : ''}</p>}
        </div>
      ...
      <div className="rounded-2xl border border-metro-border bg-metro-surface p-4 shadow-card">
        ...
        {selected.status === 'open' && <div className="mt-4 flex gap-2"><input className="min-w-0 flex-1 rounded-lg border border-metro-border bg-metro-panel px-3 py-2 text-sm text-metro-text outline-none focus:border-metro-red" onChange={(event) => setNewPoint(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void handleAddManual(); }} placeholder="Añadir otro punto al guion..." value={newPoint}/><button className="inline-flex items-center gap-2 rounded-lg bg-metro-red px-3 py-2 text-sm font-semibold text-white" onClick={() => void handleAddManual()} type="button"><Plus size={16}/>Crear punto</button></div>}
      </div>
